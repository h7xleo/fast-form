...
// 这是一个JSON指针的实现库 This is an implementation of JSON Pointer.
...
function resolveReference(
  ...
  // Retrieve the referenced schema definition. 检索引用的模式定义
  const $refSchema = findSchemaDefinition(schema.$ref, rootSchema);
  // Drop the $ref property of the source schema. 删除源模式的$ref属性
  const { $ref, ...localSchema } = schema;
  // Update referenced schema definition with local schema properties. 使用本地模式属性更新引用的模式定义
...
export function isSelect(_schema: any, rootSchema: JsonSchema = {}) {
...
export function getMatchingOption(
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    // 如果模式描述了一个对象，那么我们需要在模式中添加稍微严格一些的匹配，
    // 因为除非模式使用“requires”关键字，否则只要对象不具有冲突类型的匹配键，它就会匹配模式。
    // 要做到这一点，我们使用一个带有require数组的“anyOf”。这种增强表示，
    // 如果模式中的任何键存在于对象上并通过验证，则模式应该匹配。
